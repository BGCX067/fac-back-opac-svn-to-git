...
def record(request, record_id):
    ...
    if settings.JANGLE:
        jangle = jangle_retrieve_items([doc["id"]])
        context['jangle'] = jangle[doc["id"]]
    template = loader.get_template('discovery/record.html')
    return HttpResponse(template.render(context))
    
def jangle_retrieve_items(record_ids):
    jangle = settings.JANGLE
    jangles = {}
    for id in record_ids:
        jangle_bits = id.split(":")
        if jangle_bits[0] not in jangles:
            jangles[jangle_bits[0]] = {}
        if jangle_bits[1] not in jangles[jangle_bits[0]]:
            jangles[jangle_bits[0]][jangle_bits[1]] = {}
        if jangle_bits[2] not in jangles[jangle_bits[0]][jangle_bits[1]]:
            jangles[jangle_bits[0]][jangle_bits[1]][jangle_bits[2]] = []            
        jangles[jangle_bits[0]][jangle_bits[1]][jangle_bits[2]].append(jangle_bits[-1])
    
    responses = []
    for jangle_nick in jangles:
        jangle_url = {"host":jangle[jangle_nick]["host"]}
        for service in jangles[jangle_nick]:
            jangle_url["service"] = service
            for resource in jangles[jangle_nick][service]:
                url = jangle_url["host"]+jangle_url["service"]+"/"+resource+"/"+",".join(jangles[jangle_nick][service][resource])+"/items/"
                url += "?"+jangle[jangle_bits[0]]["formats_argument"]+"="+jangle[jangle_bits[0]]["holdings_format"]   
                try:
                    sys.stderr.write("\n"+url+"\n")
                    responses.append(urllib.urlopen(url).read())
                except:
                    pass
    
    atom_ns = "http://www.w3.org/2005/Atom"
    dlf_ns = "http://diglib.org/ilsdi/1.0"
    records = {}
    for response in responses:
        ...
        for entry in feed.findall("./{%s}entry" % atom_ns):
            ...
            for dlf in content:
                id_uri = dlf.find("{%s}bibliographic" % dlf_ns).get("id")
                parsed_url = urlparse.urlparse(id_uri)
                id = ''
                for jangle_entries in jangle:
                    parsed_host = urlparse.urlparse(jangle[jangle_entries]["host"])
                    if parsed_host.netloc == parsed_url.netloc and parsed_host.port == parsed_url.port:
                        id = jangle_entries+":".join(parsed_url.path.split("/"))
                        if id not in records:
                            records[id] = {"copies":[]}
                copy = {}

                copy["status"] = dlf.find("{%s}items/{%s}item/{%s}simpleavailability/{%s}availabilitystatus" % (dlf_ns, dlf_ns,dlf_ns, dlf_ns)).text  

                copy["message"] = dlf.find("{%s}items/{%s}item/{%s}simpleavailability/{%s}availabilitymsg" % (dlf_ns, dlf_ns,dlf_ns, dlf_ns)).text
                for due_back in dlf.findall("{%s}items/{%s}item/{%s}simpleavailability/{%s}dateavailable" % (dlf_ns, dlf_ns, dlf_ns, dlf_ns)):    
                    copy["dateavailable"] = due_back.text
                records[id]["copies"].append(copy)            
...
def get_search_results(request): 
    ...
    if settings.JANGLE:
        jangle_responses = jangle_retrieve_items(ids)
        for record in context['response']['docs']:
            if record["id"] in jangle_responses:
                record["jangle"] = {}
                record["jangle"]["response"] = jangle_responses[record["id"]]
                due_back = '9999-99-99T99:99:99-9999Z'
                for copy in jangle_responses[record["id"]]["copies"]:                    
                    if copy["status"] == 'available':
                        record["jangle"]["status"] = copy["status"]
                        break
                    elif copy.has_key("dateavailable"):
                        if copy["dateavailable"] < due_back:
                            due_back = copy["dateavailable"] 
                if "status" not in record["jangle"]:                       
                    record["jangle"]["status"] = "unavailable"
                    if due_back != '9999-99-99T99:99:99-9999Z':
                        record["jangle"]["availabilitydate"] = due_back
